...
CapFile.prototype.parse = function () {
    ...
    if (this.globalHeader.version !== CapFile.SUPPORTED_PCAP_VERSION) {
        throw Error('Unsupported PCap File version (' + this.globalHeader.version + '). ' + 'Unable to parse.');
    }

    // Restrict parsing to WLAN types.
    if (this.globalHeader.headerType !== CapFile.WLAN_HEADER_TYPE) {
        throw Error('Unsupported (non-WLAN) Pcap file header type (' + this.globalHeader.headerType + '). ' + 'Unable to parse.');
    }
...
CapFile.prototype.extractPmkFields = function (givenSsid) {
    ...
    for (i = 0; i < this.packetFrames.length; i++) {
        ...
        if (!frame.hasOwnProperty('taggedParameters') || !frame.hasOwnProperty('_bssid')) {
            continue;
        }
    ...
    for (var bssid in bssid_to_ssid) {
        ...
        for (i = 0; i < this.packetFrames.length; i++) {
            ...
            if (mic && ack && install) {
                ...
                if (CapFile.debug) {
                    CapFile.debug('Handshake (3 of 4): src: ' + hsSrcAddress + ', dst: ' + hsDstAddress + ', ANonce: ' + anonce + ', counter: ' + hsReplayCounter);
                }
                continue;
            }
