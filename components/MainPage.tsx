    ...
    ${style.heading}
      `}>
                ...
                <div className={style['theme']}>
                    {theme === 'dark' ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={Sun} alt={'light-theme'} className={`m-fl-3xs  w-fl-xs`} onClick={() => setTheme('light')} />
                    ) : (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={Moon} alt={'dark-theme'} className={`m-fl-3xs w-fl-xs`} onClick={() => setTheme('dark')} />
                    )}
                </div>
                {!isDesktop && (
                    <TetherComponent
